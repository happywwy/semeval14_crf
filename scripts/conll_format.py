...
def sent_dep(filename):
    f_Laptops = filename.read().splitlines()
    plist = []
    all_dep = []
    
    for line in f_Laptops:
...
if __name__ == "__main__":
    ...
    xml.sax.parse("Laptops_Train.xml", handler)
	

    all_deps = sent_dep(open('Laptops_Train.parse', 'r'))
    ...
    for id, sentence in sorted(handler.sentences.items()):
        ...
        features = [sentence, pos, next_word, previous_word, is_cap, head_word, dep_rel1, dep_rel2]
